...
async function notifyRoles(roles, { title, message }) {
  for (const targetRole of roles) {
    try {
      await createNotification({ type: "bmr_event", title, message, targetRole });
    } catch (e) {
      console.error("[BMR notify]", e.message);
    }
  }
}
...
router.get("/", async (req, res) => {
  try {
    const { section, status, planId } = req.query;
    const where = {};
      ...
      records.map(async (r) => {
        try {
          const plan = await prisma.productionPlan.findUnique({
            where: { id: r.planId },
            select: {
              plannedDate: true, planId: true, equipment: true,
...
router.post("/", async (req, res) => {
  try {
    ...
    res.json({ success: true, data: bmr, created: true });
  } catch (e) {
    res.status(500).json({ success: false, error: e.message });
  }
});
...
router.get("/:id", async (req, res) => {
  try {
    ...
    if (!bmr) return res.status(404).json({ success: false, error: "BMR not found" });
    ...
    res.json({ success: true, data: { ...bmr, plan } });
  } catch (e) {
    res.status(500).json({ success: false, error: e.message });
  }
});
...
router.put("/:id/section-a", async (req, res) => {
  try {
    const { id } = req.params;
    ...
    res.json({ success: true, data: sectionA });
  } catch (e) {
    res.status(500).json({ success: false, error: e.message });
  }
});
...
router.put("/:id/section-b", async (req, res) => {
  try {
    const { id } = req.params;
    const bmr = await prisma.bmrRecord.findUnique({ where: { id } });
    if (!bmr) return res.status(404).json({ success: false, error: "BMR not found" });
    ...
    const cultureRow = (c, i, sectionBId) => ({
      sectionBId,
    ...
    if (existing) {
      ...
      if (Array.isArray(cultureDetails)) {
        ...
        if (cultureDetails.length)
          await prisma.bmrCultureDetail.createMany({
            data: cultureDetails.map((c, i) => cultureRow(c, i, existing.id)),
          });
      }
    } else {
      ...
      if (Array.isArray(cultureDetails) && cultureDetails.length)
        await prisma.bmrCultureDetail.createMany({
          data: cultureDetails.map((c, i) => cultureRow(c, i, sectionB.id)),
        });
    }

    await refreshBmrStatus(id);
    await logEvent("BMR_SECTION_B_SAVED", { bmrId: id, planId: bmr.planId });
    res.json({ success: true, data: sectionB });
  } catch (e) {
    res.status(500).json({ success: false, error: e.message });
  }
});
...
router.put("/:id/section-c", async (req, res) => {
  try {
    const { id } = req.params;
    const bmr = await prisma.bmrRecord.findUnique({ where: { id } });
    if (!bmr) return res.status(404).json({ success: false, error: "BMR not found" });
    ...
    res.json({ success: true, data: sectionC });
  } catch (e) {
    res.status(500).json({ success: false, error: e.message });
  }
});
...
router.patch("/:id/section-c/checklist/:itemId", async (req, res) => {
  try {
    ...
    await logEvent("BMR_RM_CHECKLIST_TICK", {
      bmrId: id,
      payload: { itemId, rmCode: item.rmCode, added, time: new Date() },
    });
...
router.put("/:id/section-d", async (req, res) => {
  try {
    const { id } = req.params;
    const bmr = await prisma.bmrRecord.findUnique({ where: { id } });
    if (!bmr) return res.status(404).json({ success: false, error: "BMR not found" });
    ...
    res.json({ success: true, data: sectionD });
  } catch (e) {
    res.status(500).json({ success: false, error: e.message });
  }
});
...
router.put("/:id/section-e", async (req, res) => {
  try {
    const { id } = req.params;
    const bmr = await prisma.bmrRecord.findUnique({ where: { id } });
    if (!bmr) return res.status(404).json({ success: false, error: "BMR not found" });
    ...
    if (existing) {
      ...
      if (Array.isArray(parameters)) {
        ...
        if (parameters.length)
          await prisma.bmrCoaParameter.createMany({
            data: parameters.map((p, i) => paramRow(p, i, existing.id)),
          });
      }
    } else {
      ...
      if (Array.isArray(parameters) && parameters.length)
        await prisma.bmrCoaParameter.createMany({
          data: parameters.map((p, i) => paramRow(p, i, sectionE.id)),
        });
    }
    ...
    res.json({ success: true, data: sectionE });
  } catch (e) {
    res.status(500).json({ success: false, error: e.message });
  }
});
...
router.post("/:id/deviation", async (req, res) => {
  try {
    ...
    if (!section || !details?.trim())
      return res.status(400).json({ success: false, error: "section and details required" });

    const deviation = await prisma.bmrDeviation.create({
      data: { bmrId: id, section, details, supervisorName: supervisorName || null, notifiedBy: notifiedBy || null },
    });
...
router.post("/:id/sample", async (req, res) => {
  try {
    ...
    if (!sectionPrefix) return res.status(400).json({ success: false, error: "sectionPrefix required" });
...
router.patch("/:id/sample/:sampleId/send-to-qc", async (req, res) => {
  try {
    ...
    const sample = await prisma.bmrSample.update({
      where: { sampleId },
      data: { composition: composition || null, sampleStatus: "SENT_TO_QC", sentToQcAt: new Date() },
    });

    await notifyRoles(["QC_ANALYST", "QC_MANAGER"], {
      title: `New Sample for QC — ${bmr.productName}`,
      message: `Sample ${sampleId} from Batch ${bmr.batchCode} | DI: ${bmr.diNo} | Section: ${bmr.section} sent by ${sentBy || "Section"}.\nComposition: ${composition || "—"}`,
    });
...
router.patch("/:id/sample/:sampleId/verify", async (req, res) => {
  try {
    const { id, sampleId } = req.params;
    const bmr = await prisma.bmrRecord.findUnique({ where: { id } });
    if (!bmr) return res.status(404).json({ success: false, error: "BMR not found" });
    ...
    const sample = await prisma.bmrSample.update({
      where: { sampleId },
      data: { sampleStatus: "VERIFIED", verifiedAt: new Date(), verifiedBy: verifiedBy || null },
    });

    await createNotification({
      type: "bmr_sample_verified",
      title: `Sample Verified — ${sampleId}`,
      message: `QC verified Sample ${sampleId} for Batch ${bmr.batchCode}. Result: ${result || "See COA"}.`,
      targetRole: "PRODUCTION",
    });
