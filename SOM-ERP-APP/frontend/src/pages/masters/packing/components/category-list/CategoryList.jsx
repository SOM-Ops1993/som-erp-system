...
import { CATEGORIES, SUB_TYPES } from '../packing-constants/packingConstants.jsx'
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      {CATEGORIES.map(cat => (
        ...
        >
          ...
          <div className={`mb-4 mt-1 ${cat.cls.text}`}><cat.icon size={34} strokeWidth={1.6} /></div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {(SUB_TYPES[cat.value] || []).map(s => (
              <span key={s.value} className={`inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full font-medium border-0 ${cat.cls.badge}`}>
                <s.icon size={11} />{s.value}
              </span>
            ))}
          </div>
        </button>
